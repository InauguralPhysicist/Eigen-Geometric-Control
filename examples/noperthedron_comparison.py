...
def create_comparison_visualization(arm_results, nop_results, save_path='noperthedron_comparison.png'):
    ...
    ax1.fill_between(arm_results['iterations'], 0, ax1.get_ylim()[1],
                     alpha=0.2, color='green', label='Time-like (convergent)')
    ax1.fill_between(arm_results['iterations'], ax1.get_ylim()[0], 0,
                     alpha=0.2, color='yellow', label='Space-like (non-convergent)')
    ...
    ax2.plot(arm_results['iterations'], arm_results['C'], label='C (Change)', color='red', linewidth=2)
    ax2.plot(arm_results['iterations'], arm_results['S'], label='S (Stability)', color='green', linewidth=2)
    ...
    sc = ax3.scatter(arm_results['C'], arm_results['S'],
                     c=arm_results['iterations'], cmap='viridis',
                     s=30, alpha=0.6, edgecolors='black', linewidth=0.5)
    ax3.plot([0, max(arm_results['C'])], [0, max(arm_results['C'])],
             'k--', alpha=0.3, label='C = S (ds² = 0)')
    ...
    ymax = ax4.get_ylim()[1]
    ax4.fill_betweenx([0, ymax], 0, max(nop_results['ds2']),
                      alpha=0.2, color='green', label='Time-like')
    ax4.fill_betweenx([0, ymax], min(nop_results['ds2']), 0,
                      alpha=0.2, color='yellow', label='Space-like')
    ax4.set_xlabel('ds²')
    ...
    ax5.scatter(nop_results['C'], nop_results['S'],
    ...
    ax6.axhline(frac_spacelike, color='orange', linestyle=':', alpha=0.7,
    ...
    ax7.bar(x - width/2, [arm_timelike, arm_spacelike],
            width, label='Arm Control', color='blue', alpha=0.7, edgecolor='black')
    ax7.bar(x + width/2, [nop_timelike, nop_spacelike],
            width, label='Noperthedron', color='purple', alpha=0.7, edgecolor='black')
...
Geometry determines dynamics!
    ...
    ax9.text(0.1, 0.5, theory_text, fontsize=9, verticalalignment='center',
             family='monospace', bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.3))

    # Main title
    fig.suptitle('Noperthedron vs Arm Control: Unified Geometric Framework',
