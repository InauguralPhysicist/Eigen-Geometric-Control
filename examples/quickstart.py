...
def main():
    ...
    print(f"  Min obstacle dist: {min_obstacle_dist:.3f} m (safe: >{results['d_obs'].iloc[0] - 0.25:.3f} m)")
    print()
...
if __name__ == "__main__":
    ...
    try:
        ...
        ax1.plot(results['x'].iloc[0], results['y'].iloc[0], 'go', 
                markersize=10, label='Start')
        ax1.plot(results['x'].iloc[-1], results['y'].iloc[-1], 'ro', 
                markersize=10, label='End')
        ax1.plot(1.2, 0.3, 'r*', markersize=15, label='Target')
        
        # Add obstacle circle
        circle = plt.Circle((0.6, 0.1), 0.25, fill=False, 
                           color='red', linestyle='--', linewidth=2, label='Obstacle')
