...
class TestErrorHandling:
    ...
    def test_compute_ds2_negative_params(self):
        ...
        with pytest.raises(ValueError, match="Go must be non-negative"):
            compute_ds2(
                0, 0, target=[1.0, 0.0], obstacle_center=[0.5, 0.5], obstacle_radius=0.2, Go=-1.0
            )

        with pytest.raises(ValueError, match="lam must be non-negative"):
            compute_ds2(
                0, 0, target=[1.0, 0.0], obstacle_center=[0.5, 0.5], obstacle_radius=0.2, lam=-0.1
            )
...
class TestNumericalAccuracy:
    ...
    def test_obstacle_repulsion_activates(self):
        ...
        ds2_far, comps_far = compute_ds2(
            theta1, theta2, target, obstacle_center=np.array([0, 5.0]), obstacle_radius=0.5
        )

        # Obstacle close
        ds2_near, comps_near = compute_ds2(
            theta1, theta2, target, obstacle_center=np.array([1.5, 0.0]), obstacle_radius=1.0
        )
...
class TestPhysicalConstraints:
    ...
    def test_regularization_penalizes_large_angles(self):
        ...
        ds2_small, comps_small = compute_ds2(
            ...
            obstacle_radius,
            Go=0.0,
            lam=1.0,  # Only regularization active
        )

        # Large angles
        ds2_large, comps_large = compute_ds2(
            1.0, 1.0, target, obstacle_center, obstacle_radius, Go=0.0, lam=1.0
        )
